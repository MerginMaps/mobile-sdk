...
function shouldbuild_gsl() {
  ...
  if [ $BUILD_PATH/gsl/build-$ARCH/.libs/libgsl.a -nt $BUILD_gsl/.patched ]; then
    DO_BUILD=0
  fi
}
...
function build_gsl() {
  ...
  export CFLAGS="${CFLAGS} -I$BUILD_gsl/matrix"
  try $BUILD_gsl/configure --prefix=$STAGE_PATH --host=${TOOLCHAIN_PREFIX} --disable-shared
  try $MAKESMP
  try make install
...
function postbuild_gsl() {
  LIB_ARCHS=`lipo -archs ${STAGE_PATH}/lib/libgsl.a`
