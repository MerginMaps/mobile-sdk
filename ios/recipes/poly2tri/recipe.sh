...
function shouldbuild_poly2tri() {
  ...
  if [ ${STAGE_PATH}/include/poly2tri/poly2tri.h -nt $BUILD_poly2tri/.patched ]; then
    DO_BUILD=0
  fi
}
...
function build_poly2tri() {
  ...
  try mkdir -p ${STAGE_PATH}/include/poly2tri/sweep
  try cp -R src/3rdparty/poly2tri/sweep/*.h ${STAGE_PATH}/include/poly2tri/sweep/
  try mkdir -p ${STAGE_PATH}/include/poly2tri/common
